...
def plot_data_compare_2(interval1,data1_1, data1_2, data1_3, 
                      interval2, data2_1, data2_2, data2_3,
                      names1, style, p_title1, names2, p_title2, arb_i, arb_d):

  close='all'
  if ( style == 0): 


#    integral_arb= np.zeros(1)  
#    integral_dy= np.zeros(1)  
#    for i in np.arange(len(interval1)):
#      integral_dy=integral_dy+data1_3[i]*0.05
#    
#    for i in np.arange(len(arb_i)-1):
#      integral_arb=integral_arb +(arb_i[i+1]-arb_i[i])*arb_d[i]
    ...
    arb_d=arb_d*50/18
    ax.plot(arb_i,arb_d, color='purple',linestyle=':',linewidth=2,label='l_nodiff re200')
